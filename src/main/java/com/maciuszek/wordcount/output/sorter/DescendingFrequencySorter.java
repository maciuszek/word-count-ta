...
public class DescendingFrequencySorter implements Sorter<Flux<WordCount>> {
    ...
    public Flux<WordCount> sort(Flux<WordCount> flux) {
        ...
        }) // for production the sorting algorithm would need to leverage a database that can automatically scale since this assumes an undefined amount of memory. a better solution would be active sorting as attempted with https://github.com/maciuszek/word-count-ta/blob/active_sort/src/main/java/com/maciuszek/wordcount/service/ActiveSortingCountService.java
